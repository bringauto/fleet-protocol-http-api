        ...
        with self.app.app.test_client() as client:
            for invalid_car_name in self.invalid_car_names:
                with self.subTest(invalid_car_name=invalid_car_name):
                    response = client.get(
                        f"/status/valid_company_name/{invalid_car_name}", json=[self.status]
                    )
                    print(response.json)
            ...
            for invalid_company_name in self.invalid_company_names:
                with self.subTest(invalid_company_name=invalid_company_name):
                    response = client.get(
                        f"/status/{invalid_company_name}/valid_car_name", json=[self.status]
                    )
                    self.assertEqual(response.status_code, 400)
                    self.assertIn("pattern", response.json["detail"])  # type: ignore
    ...
    def test_command_endpoint(self) -> None:
        with self.app.app.test_client() as client:
            for invalid_car_name in self.invalid_car_names:
                with self.subTest(invalid_car_name=invalid_car_name):
                    response = client.get(
                        f"/command/valid_company_name/{invalid_car_name}", json=[self.status]
                    )
                    self.assertEqual(response.status_code, 400)
                    self.assertIn("pattern", response.json["detail"])  # type: ignore
            for invalid_company_name in self.invalid_company_names:
                with self.subTest(invalid_company_name=invalid_company_name):
                    response = client.get(
                        f"/command/{invalid_company_name}/valid_car_name", json=[self.status]
                    )
                    self.assertEqual(response.status_code, 400)
                    self.assertIn("pattern", response.json["detail"])  # type: ignore
    ...
    def test_available_device_endpoint(self) -> None:
        with self.app.app.test_client() as client:
            for invalid_car_name in self.invalid_car_names:
                with self.subTest(invalid_car_name=invalid_car_name):
                    response = client.get(
                        f"/available-devices/valid_company_name/{invalid_car_name}",
                        json=[self.status],
                    )
                    self.assertEqual(response.status_code, 400)
                    self.assertIn("pattern", response.json["detail"])  # type: ignore
            for invalid_company_name in self.invalid_company_names:
                with self.subTest(invalid_company_name=invalid_company_name):
                    response = client.get(
                        f"/available-devices/{invalid_company_name}/valid_car_name",
                        json=[self.status],
                    )
                    self.assertEqual(response.status_code, 400)
                    self.assertIn("pattern", response.json["detail"])  # type: ignore
