...
def clean_up_disconnected_cars_and_modules() -> None:
    ...
    for company_name in _device_ids:
        for car_name in _device_ids[company_name]:
            empty_modules = [module_id for module_id in _device_ids[company_name][car_name].keys() if not _device_ids[company_name][car_name][module_id]]
            for module_id in empty_modules:
                _device_ids[company_name][car_name].pop(module_id)

        cars_without_modules = [car for car in _device_ids[company_name].keys() if not _device_ids[company_name][car]]
